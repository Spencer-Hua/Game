    ...
    def update(self, player):
        ...
        if self.move_pattern == 0:  # 左右巡逻
            ...
            if player.x > self.x:
                ...
                self.x -= self.vel_x
                self.direction = -1
        elif self.move_pattern == 2:  # 跳跃移动
            if self.is_on_ground() and current_time % 2000 < 50:
                self.vel_y = -10
        ...
        # 地面碰撞检测
        if self.y > SCREEN_HEIGHT - 200 - self.height:
            self.y = SCREEN_HEIGHT - 200 - self.height
            self.vel_y = 0
        
        # 攻击模式切换
        if current_time - self.attack_timer > 2000:  # 每2秒切换一次攻击模式
...
def create_level():
    ...
    goal = Platform(SCREEN_WIDTH - 45, 50, 45, 45, YELLOW)  # 放置在右上角
...
def create_level2():
    ...
    goal = Platform(SCREEN_WIDTH - 45, 50, 45, 45, YELLOW)
...
def create_boss_level():
    ...
    platforms = [
        ...
        # 战斗平台
        Platform(200, 700, 150, 30),
        Platform(500, 650, 200, 30),
        Platform(850, 700, 150, 30),
        
        # 高处平台
        Platform(300, 500, 100, 30),
        Platform(600, 450, 200, 30),
        Platform(900, 500, 100, 30),
    ]
    ...
    return platforms, spikes, static_spikes, checkpoints, goal, coins, boss, bullets
...
def create_random_level():
    ...
    for platform in platforms[1:]:  # 不在地面上生成动态尖刺
        ...
        # 将生成概率从30%提升到50%
        if random.random() > 0.5:  # 50%概率生成动态尖刺
    ...
    for _ in range(5):  
        ...
        while attempts < 100:
            ...
            direction = random.choice(["up", "down", "left", "right"])
            new_spike = StaticSpike(x, y, 30, 30, direction)
            ...
            on_platform = False
            for p in platforms:
                if (new_spike.x < p.x + p.width and
                    new_spike.x + new_spike.width > p.x and
                    new_spike.y < p.y + p.height and
                    new_spike.y + new_spike.height > p.y):
                    on_platform = True
                    break
            ...
            overlap = False
            for s in spikes + static_spikes:
                if (new_spike.x < s.x + s.width and
                    new_spike.x + s.width > s.x and
                    new_spike.y < s.y + s.height and
                    new_spike.y + s.height > s.y):
                    overlap = True
                    break
            
            if on_platform and not overlap:
                static_spikes.append(new_spike)
                break
            attempts += 1
...
def create_tutorial_level():
    ...
    # 修改为黄色方形区域作为通关目标
    goal = Platform(SCREEN_WIDTH - 45, 50, 45, 45, YELLOW)  # 使用黄色方形区域表示通关位置
    ...
    return platforms, spikes, static_spikes, checkpoints, goal, coins, tutorial_texts
...
if selected_level == "tutorial":
    ...
    platforms, spikes, static_spikes, checkpoints, goal, coins, boss, bullets= create_boss_level()
    player = Player(50, 600)
    game_state = "playing"
    start_ticks = pygame.time.get_ticks()
    boss_defeated = False
else:  # 处理随机生成的关卡
    platforms, spikes, static_spikes, checkpoints, goal, coins, bullets = create_random_level()
    player = Player(50, 600)
    game_state = "playing"
    start_ticks = pygame.time.get_ticks()
...
while running:
    ...
    if not player.alive:
        ...
        if selected_level == "boss":
            ...
            boss.vel_x, boss.vel_y = 2, 0  # 重置速度
            boss.move_pattern = 0
