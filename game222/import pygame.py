...
try:
    ...
    font_path = os.path.join(resource_path, "华康方圆体W7.TTC")  # 字体文件放在同级目录下
...
skull_img=pygame.image.load(os.path.join(resource_path, "images", "dead.png")).convert_alpha()
skull_img=pygame.transform.scale(skull_img, (30, 30))

coin_img=pygame.image.load(os.path.join(resource_path, "images", "coin.gif")).convert_alpha()  # 修改为使用resource_path
coin_img=pygame.transform.scale(coin_img, (30, 30))

save1_image=pygame.image.load(os.path.join(resource_path, "images", "save1.png")).convert_alpha()
save1_image=pygame.transform.scale(save1_image, (40, 40))

save2_image=pygame.image.load(os.path.join(resource_path, "images", "save2.png")).convert_alpha()
save2_image=pygame.transform.scale(save2_image, (40, 40))
...
class Checkpoint:
    ...
    def draw(self, screen):
        # 根据激活状态选择图像
        if self.activated:
            screen.blit(save2_image, (self.x, self.y))  # 显示第二张图像
        else:
            screen.blit(save1_image, (self.x, self.y))  # 显示第一张图像
...
class Cloud:
    def __init__(self, x, y, speed):
        self.x = x
        self.y = y
        self.image = pygame.image.load("images/cloud.png").convert_alpha()  # 加载云朵图片
        self.image = pygame.transform.scale(self.image, (120, 72))  # 调整云朵大小
        self.speed = speed  # 云朵移动速度
    ...
    def draw(self, screen):
        ...
        screen.blit(self.image, (self.x, self.y))
clouds = []
for _ in range(4):  # 创建5朵云朵
    cloud_x = random.randint(-200, SCREEN_WIDTH + 200)  # 云朵X坐标范围更大，分散到屏幕外
    cloud_y = random.randint(50, SCREEN_HEIGHT - 200)  # 云朵Y坐标范围更大，避免太高或太低
    cloud_speed = random.randint(1, 2)  # 随机设定云朵的速度
    cloud = Cloud(cloud_x, cloud_y, cloud_speed)
    clouds.append(cloud)
def show_game_over_screen():
    """显示游戏结束界面"""
    game_over_font = pygame.font.Font(font_path, 48)
    restart_font = pygame.font.Font(font_path, 36)
...
def show_game_win_screen():
    """显示游戏胜利界面"""
    win_font = pygame.font.Font(font_path, 48)
    restart_font = pygame.font.Font(font_path, 36)
...
def show_level_selection():
    ...
    while selecting:
        ...
        title_text = pygame.font.Font(font_path, 48).render("选择关卡", True, WHITE)
        screen.blit(title_text, (SCREEN_WIDTH//2 - title_text.get_width()//2, 100))

        mouse_pos = pygame.mouse.get_pos()

        # 新手教程
        tutorial_text_base = pygame.font.Font(font_path, 36).render("新手教程", True, LIGHT_BLUE)
        tutorial_rect = tutorial_text_base.get_rect(topleft=(SCREEN_WIDTH//2 - tutorial_text_base.get_width()//2, 170))
        tutorial_color = YELLOW if tutorial_rect.collidepoint(mouse_pos) else LIGHT_BLUE
        tutorial_text = pygame.font.Font(font_path, 36).render("新手教程", True, tutorial_color)
        screen.blit(tutorial_text, tutorial_rect)

        # 关卡1
        level1_base = pygame.font.Font(font_path, 36).render("关卡1", True, LIGHT_BLUE)
        level1_rect = level1_base.get_rect(topleft=(SCREEN_WIDTH//2 - level1_base.get_width()//2, 225))
        level1_color = YELLOW if level1_rect.collidepoint(mouse_pos) and unlocked_levels["level1"] else (LIGHT_BLUE if unlocked_levels["level1"] else DARK_GRAY)
        level1_text = pygame.font.Font(font_path, 36).render("关卡1", True, level1_color)
        screen.blit(level1_text, level1_rect)

        # 关卡2
        level2_base = pygame.font.Font(font_path, 36).render("关卡2", True, LIGHT_BLUE)
        level2_rect = level2_base.get_rect(topleft=(SCREEN_WIDTH//2 - level2_base.get_width()//2, 300))
        level2_color = YELLOW if level2_rect.collidepoint(mouse_pos) and unlocked_levels["level2"] else (LIGHT_BLUE if unlocked_levels["level2"] else DARK_GRAY)
        level2_text = pygame.font.Font(font_path, 36).render("关卡2", True, level2_color)
        screen.blit(level2_text, level2_rect)

        # BOSS挑战
        boss_base = pygame.font.Font(font_path, 36).render("BOSS挑战", True, LIGHT_BLUE)
        boss_rect = boss_base.get_rect(topleft=(SCREEN_WIDTH//2 - boss_base.get_width()//2, 375))
        boss_color = YELLOW if boss_rect.collidepoint(mouse_pos) and unlocked_levels["boss"] else (LIGHT_BLUE if unlocked_levels["boss"] else DARK_GRAY)
        boss_text = pygame.font.Font(font_path, 36).render("BOSS挑战", True, boss_color)
        screen.blit(boss_text, boss_rect)

        # 随机关卡
        random_base = pygame.font.Font(font_path, 36).render("随机关卡", True, LIGHT_BLUE)
        random_rect = random_base.get_rect(topleft=(SCREEN_WIDTH//2 - random_base.get_width()//2, 450))
        random_color = YELLOW if random_rect.collidepoint(mouse_pos) else LIGHT_BLUE
        random_text = pygame.font.Font(font_path, 36).render("随机关卡", True, random_color)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    if tutorial_rect.collidepoint(mouse_pos):
                        ...
                        selecting = False
                        return "level2"
                    elif random_rect.collidepoint(mouse_pos):  # 新增：处理随机关卡选择
                        selecting = False
                        return "random"
...
while running:
    ...
    # 渲染文字
    death_text = pygame.font.Font(font_path, 24).render(f"死亡数: {player.death_count}", True, WHITE)
    coin_text = pygame.font.Font(font_path, 24).render(f"硬币: {collected_coins}/5", True, WHITE)

    # 获取文字尺寸，用于图标定位
    text_y = 10
    death_text_x = 10
    coin_text_x = death_text_x + death_text.get_width() + 40  # 给图标留空间

    # 绘制图标和文字
    screen.blit(skull_img, (death_text_x + death_text.get_width() + 5, text_y))
    screen.blit(coin_img, (coin_text_x + coin_text.get_width() + 5, text_y))

    screen.blit(death_text, (death_text_x, text_y))
    screen.blit(coin_text, (coin_text_x, text_y))
    
    # 新增计时器逻辑
    current_time = pygame.time.get_ticks() - start_ticks
    timer_text = pygame.font.Font(font_path, 18).render(f"游戏时间: {current_time // 1000}秒", True, WHITE)
    screen.blit(timer_text, (10, 40))  # 在左上角绘制计时器
    
    if game_state == "win":
        win_text = pygame.font.Font(font_path, 24).render("恭喜通关! 按 esc 返回开始界面", True, YELLOW)
    ...
    for i, text in enumerate(controls):
        ctrl_text = pygame.font.Font(font_path, 24).render(text, True, LIGHT_BLUE)
